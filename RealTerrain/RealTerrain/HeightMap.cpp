...
{
	auto matToVec = [&](int x, int y)
	{
		return x * size.second + y;
	};
	...
	for (x = 1; x < size.first; x++)
		for (z = 0; z < size.second; z++)
			heightMap[x][z] = heightMap[x - 1][z] * (1.f - factor) +
				heightMap[x][z] * factor;

	for (x = size.first - 2; x < -1; x--)
		for (z = 0; z < size.second; z++)
			heightMap[x][z] = heightMap[x + 1][z] * (1 - factor) +
				heightMap[x][z] * factor;

	for (x = 0; x < size.first; x++)
		for (z = 1; z < size.second; z++)
			heightMap[x][z] = heightMap[x][z - 1] * (1 - factor) +
			heightMap[x][z] * factor;
