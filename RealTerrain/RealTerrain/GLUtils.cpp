...
std::shared_ptr<Mesh> ParseObj(std::ifstream& file)
{
    ...
    while (!file.eof())
    {
        ...
        if (line[0] == '#' || line[0] == 's' || line.substr(0, 6) == "usemtl" || line.substr(0, 6) == "mtllib")
            continue;
        ...
        for (size_t tIndex = 1; tIndex <= 3; tIndex++)
        {
            ...
            std::size_t pos = std::stoi(elements[0]) - 1, tex = std::stoi(elements[1]) - 1,
                        norm = std::stoi(elements[2]) - 1;
